...
class AgenteComparativa:
    ...
    def obtener_puntos_labores(self, finca_id, persona_id, fecha, version='v715'):
        if version == 'v715':
            tablas = ['l_pal_polins']
        else:
...
def comparar():
    ...
    version = datos.get('version', 'v715') # Default to v715 for this tool
    
    comparativa = agente.generar_comparativa(finca_id, desde, hasta, version=version)
...
def descargar_reporte():
    try:
        ...
        version = request.args.get('version', 'v715')
        ...
        cols = ['fecha', 'PERSONA', 'FUNC', 'LOTE', 'labor_nombre', v_col_i, v_col_f, 'mapa_linea_i', 'mapa_linea_f', 'estado']
...
def puntos_labores(finca_id, persona_id, fecha):
    return jsonify(agente.obtener_puntos_labores(finca_id, persona_id, fecha, version='v715'))
